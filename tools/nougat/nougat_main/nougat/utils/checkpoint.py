...
def get_checkpoint(
    checkpoint_path: Optional[os.PathLike] = None, download: bool = False
) -> Path:
    """
    ...
    print('checkpoint', checkpoint,'is_exists', checkpoint.exists(),'is_file', checkpoint.is_file())
    ...
    if download and (not checkpoint.exists() or len(os.listdir(checkpoint)) < 5):
        ...
        print('checkpoint:', checkpoint, ' ,is_exists:', checkpoint.exists(), ' ,is_file', checkpoint.is_file(),' ,please check or set download to True')
        return None
    return checkpoint
