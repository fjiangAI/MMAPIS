...
#             args, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=20
#         )
#         result.check_returncode()  # This will raise a CalledProcessError if the command failed
#
#         json_file = os.path.join(TEMP_DIR, os.path.basename(pdf_file).replace(".pdf", ".json"))
#         if os.path.exists(json_file):
#             with open(json_file, "rb") as f:
#                 data = json.load(f)
#             return data
#         else:
#             logging.error(f"No JSON output file found for {pdf_file}")
#             return None
#     except subprocess.CalledProcessError as e:
#         logging.error(f"Failed to run pdffigures2: {e}")
#         return None
#     except Exception as e:
#         logging.error(f"Failed to parse PDF using pdffigures2: {e}")
#         return None
...
#         logging.info(f"Temp directory {TEMP_DIR} removed")
