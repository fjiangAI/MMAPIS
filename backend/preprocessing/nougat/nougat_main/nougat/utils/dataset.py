...
class LazyDataset(Dataset):
    ...
    """
    def __init__(self, pdf:Union[Path,str,bytes], prepare: Callable,proxy=None,pdf_name:str = None,headers=None):
        super().__init__()
        self.prepare = prepare
        # self.init_fn: list of images from 17pdf
        self.init_fn = partial(rasterize_paper, pdf,proxy=proxy,headers=headers)
        self.dataset = None
        _,self.size, self.name = get_pdf_doc(pdf,proxy=proxy,headers=headers,pdf_name=pdf_name)
