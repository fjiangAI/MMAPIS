...
def nougat_predict(args,proxy=None,headers=None,pdf_name:str = None):
        ...
        if isinstance(pdf,Path):
            ...
            if args.out:
                ...
                if out_path.exists() and not args.recompute:
                    ...
                    continue
        try:
        # 17pdf: Path, prepare_input: Callable[[Image.Image], torch.Tensor]
        # input: 17pdf + prepare_input(img)
        # dataset: [num_pages]
            dataset = LazyDataset(
    ...
    file_index = 0
    page_num = 0
    # [sample(img) , is_last_page] is data from a 17pdf file (1 page)
