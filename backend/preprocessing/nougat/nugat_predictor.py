import os
import sys
from MMAPIS.backend.config.config import NOUGAT_CONFIG, GENERAL_CONFIG
from MMAPIS.backend.tools.utils import get_batch_size,torch_gc
import urllib.error
...
from MMAPIS.backend.preprocessing.nougat.nougat_main.nougat import NougatModel
from MMAPIS.backend.preprocessing.nougat.nougat_main.nougat.utils.dataset import LazyDataset
from MMAPIS.backend.preprocessing.nougat.nougat_main.nougat.postprocessing import markdown_compatible
from MMAPIS.backend.preprocessing.nougat.nougat_main.nougat.utils.checkpoint import get_checkpoint
...
class NougatPredictor:
    def __init__(self,
        ...
        self.checkpoint = get_checkpoint(checkpoint)
            ...
            for j, output in enumerate(model_output["predictions"]):
                ...
                if is_last_page[j]:
                    ...
                    if self.out:
                        ...
                        formatted_pdf_name = str(file_name).rsplit(".", 1)[0].replace('.', '_')
                        files.append(NougatArticle(file_name=formatted_pdf_name, content=out))
                    else:
                        print(out, "\n\n")
                    predictions = []
                    page_num = 0
