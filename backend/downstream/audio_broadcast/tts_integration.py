from MMAPIS.backend.downstream.audio_broadcast.broadcast_script_generator import BroadcastScriptGenerator
from MMAPIS.backend.downstream.audio_broadcast.tts import YouDaoTTSConverter, OpenAITTSConverter
from typing import List, Union
from MMAPIS.backend.config.config import TTS_CONFIG,OPENAI_CONFIG, APPLICATION_PROMPTS
import reprlib
import os
import logging
...
class BroadcastTTSGenerator():
    def __init__(self,
        ...
        self.broadcast_generator = BroadcastScriptGenerator(api_key=llm_api_key,
        ...
        if tts_model == "youdao":
            ...
            self.tts_converter = OpenAITTSConverter(base_url=llm_base_url, api_key=llm_api_key)
        else:
    ...
    def text2speech(self, text:str, return_bytes:bool = False):
        if self.tts_model == "youdao" and not all([self.tts_converter.tts_generator.base_url,self.tts_converter.tts_generator.api_key,self.tts_converter.tts_generator.app_secret]):
