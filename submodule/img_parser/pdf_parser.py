...
class PDFPaperParser:
    ...
    def get_section_titles(self, withlevel=False, verbose=False):
        ...
        try:
            ...
            logging.warning(f"Warning: {self.path} has no abstract or reference.")
            return []
    ...
    def get_section_imagedict(self, verbose=False, snap_with_caption=SNAP_WITH_CAPTION):
        ...
        try:
            ...
            logging.warning(f"Warning: {self.path} has no image ,error:{e}")
            return {}
...
class PDFFigure2PaperParser(PDFPaperParser):
    ...
    def get_abstract(self):
        ...
        try:
            ...
            logging.warning(f"Warning: {self.path} has no abstract ,error:{e}")
            return super().get_abstract()
    ...
    def get_section_imagedict(self, snap_with_caption=SNAP_WITH_CAPTION, verbose=False):
        ...
        try :
            ...
            if verbose:
                print('Images match the content:', match_cnt)
                # return [title 1,img_idx],...
            return section_dict
        except Exception as e:
            logging.warning(f"Warning: {self.path} has no image ,error:{e}")
            return super().get_section_imagedict()


def gen_image(paper, snap_with_caption=SNAP_WITH_CAPTION, verbose=False,get_tmpfile_dir:bool = False):
    """
    Generate image for each section in xmind (Figure/Table)
    """
    section_names = paper.get_section_titles()
    img_dict = paper.get_section_imagedict(
        snap_with_caption=snap_with_caption, verbose=verbose)
    res_img_ls = defaultdict(list)
    for name in section_names[:-1]:
        img_ls = img_dict.get(name)
        if img_ls:
            for img in img_ls:
                img_tempdir = get_objpixmap(paper.pdf, img,get_tmpfile_dir=get_tmpfile_dir)
                img_tempdir = pixmap_to_img(img_tempdir)
                res_img_ls[name].append(img_tempdir)
    return res_img_ls

def pixmap_to_img(pixmap):
    image = Image.frombytes("RGB", [pixmap.width, pixmap.height], pixmap.samples)
    return image

def parser_img_from_pdf(pdf_path:str,save_dir:str=None,header:bool=True)->dict:
    pdf_parser = PDFFigure2PaperParser(pdf_path)
    img_ls = gen_image(pdf_parser)
    if save_dir is None:
        save_dir = os.path.join(os.path.dirname(pdf_path),"img")
    ...
    if img_ls:
        if header:
            img_paths = []
            img_ls = chain.from_iterable(img_ls.values())
            for i,img in enumerate(img_ls):
                img_path = os.path.join(save_dir,f"{i}.png")
                img.save(img_path)
                img_paths.append(img_path)
            logging.info(f"parser success: {len(img_paths)} img found in {pdf_path}")
            return {"img":img_paths}
        else:
            img_paths = defaultdict(list)
            for section,img_ls in img_ls.items():
                for i,img in enumerate(img_ls):
                    img_path = os.path.join(save_dir,f"{section}_{i}.png")
                    img.save(img_path)
                    img_paths[section].append(img_path)
            logging.info(f"parser success: {len(img_paths)} img found in {pdf_path}")
            return dict(img_paths)


    else:
        logging.error(f"parser error: no img found in {pdf_path}")
        return {}


if __name__ == '__main__':
    pdf_path = '../../data/test/1706.03762.pdf'
    pdf_parser = PDFFigure2PaperParser(pdf_path)
    x = gen_image(pdf_parser,snap_with_caption=SNAP_WITH_CAPTION)
    for key in x.keys():
        print(key)
        for img in x[key]:
            plt.imshow(img)
