...
def get_model_predcit(_proxy = None,_headers = None,pdf_name:str=None,**kwargs):
    ...
    _args = dict(**kwargs)
    # Path to the str
    _args['checkpoint'],_args['out'] = str(_args['checkpoint']),str(_args['out'])
    if not isinstance(_args['pdf'],list):
        _args['pdf'] = [_args['pdf']]
    print("args:",type(_args['pdf'][0]))
    if isinstance(_args['pdf'][0],bytes):
        logger.info(f"pdf_content :{len(_args['pdf'])},pdf_name:{pdf_name}")
        url = var_req_url + '/bytes_predict/'
        data = {
            "pdf_name": pdf_name,
        }
        response = requests.post(url, files={'pdf_content': _args['pdf'][0]},data=data)
    else:
        ...
        url = var_req_url + '/model_predict/'
        response = requests.post(url, json=params)
        json_info = custom_response_handler(response,func_name='model_predict')
        if not "error" in json_info:
            return json_info['article_ls'], json_info['file_names']
        else:
            return None,json_info.get('error',"error")
...
def get_summary(
                ...
                init_grid:int = 2,
                **kwargs):
    url = var_req_url + '/get_summaries/'
    summerizer_config = dict(**kwargs)
    params = {
        "api_key": api_key,
        "artile_text": article,
        "file_name": file_name,
        "init_grid": init_grid,
        "summarizer_config": summerizer_config,
    }
    params = dict_filter_none(params)
    logging.info(f'get summary params:\n{params}')
    response = requests.post(url, json=params)
    json_info = custom_response_handler(response,func_name='get_summary')
    if not "error" in json_info:
        return json_info['titles'], json_info['authors'], json_info['affiliations'], json_info['total_resp'], \
            json_info['re_respnse']
    else:
        return None, None, None, None, json_info.get('error',"error")
...
def display_result(answer_message,
    ...
    try:
        respnse, score = re_respnse
        temp_time = datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
        random_num = random.randint(0, 100000)
        answer_message.text_area("Summary:", f"{respnse.strip()}", height=summary_box_height,
                                 key=f"summary_{pdf_name}_{temp_time}_{random_num}")
        answer_message.text_area("Score:", f"{score}", height=summary_box_height,
                                 key=f"score_{pdf_name}_{temp_time}_{random_num}")
        answer_message.caption(f"not enough? click the button to regenerate summary")

    except:
        answer_message.text_area("Error:",f"due to unknown reason,summary generate failed,re_respnse:{re_respnse}",
